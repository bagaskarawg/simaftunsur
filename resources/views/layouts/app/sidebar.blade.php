...
<div class="min-h-screen flex" x-data="{ sidebarTerbuka: false }">
    ...
    <aside class="w-60 shrink-0 bg-white border-r border-slate-200 flex flex-col fixed inset-y-0 z-30
        ...
        <nav class="flex-1 overflow-y-auto py-3 text-sm">
            ...
            {{-- Penyaringan Kandidat — daftar mahasiswa yang memenuhi syarat program --}}
            @can('program.saring')
                <a href="{{ route('penyaringan.index') }}" wire:navigate
                   wire:current="!bg-primary-50 !text-primary-700 !border-primary-700"
                   class="flex items-center gap-3 px-4 py-2 text-sm font-medium border-l-2 border-transparent text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
            ...
            {{-- Program & Persyaratan — CRUD definisi program + syarat --}}
            @can('program.lihat')
            ...
            {{-- Kategori Klaster — master katalog label + rekomendasi pembinaan --}}
            @can('kategori-klaster.lihat')
                <a href="{{ route('kategori-klaster.index') }}" wire:navigate
                   wire:current="!bg-primary-50 !text-primary-700 !border-primary-700"
                   class="flex items-center gap-3 px-4 py-2 text-sm font-medium border-l-2 border-transparent text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
            ...
            {{-- Kegiatan & Organisasi — SKKM fitur F6 --}}
            @can('kegiatan.lihat')
            ...
            {{-- Pengabdian & Hibah — SKKM fitur F7 --}}
            ...
            {{-- Beasiswa — modul pendukung aktif --}}
            @can('beasiswa.lihat')
                <a href="{{ route('beasiswa.index') }}" wire:navigate
                   wire:current="!bg-primary-50 !text-primary-700 !border-primary-700"
                   class="flex items-center gap-3 px-4 py-2 text-sm font-medium border-l-2 border-transparent text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 7.5h1.5m-1.5 3h1.5m-7.5 3h7.5m-7.5 3h7.5m3-9V21a1.5 1.5 0 01-1.5 1.5H5.25A1.5 1.5 0 013.75 21V3.75A1.5 1.5 0 015.25 2.25h9.75M16.5 7.5v-4.5l4.5 4.5h-4.5z"/>
                    </svg>
                    Beasiswa
                </a>
            @endcan

            {{-- KKN — modul pendukung aktif --}}
            @can('kkn.lihat')
            ...
            @if (auth()->user()?->punyaPeran('admin'))
                ...
                <a href="{{ route('peran.index') }}" wire:navigate
                   wire:current="!bg-primary-50 !text-primary-700 !border-primary-700"
                   class="flex items-center gap-3 px-4 py-2 text-sm font-medium border-l-2 border-transparent text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
            ...
            @if (auth()->user()?->punyaPeran('admin'))
                ...
                <a href="{{ route('log-aktivitas.index') }}" wire:navigate
                   wire:current="!bg-primary-50 !text-primary-700 !border-primary-700"
                   class="flex items-center gap-3 px-4 py-2 text-sm font-medium border-l-2 border-transparent text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25z"/>
        ...
        <div class="border-t border-slate-200 p-3">
            <a href="{{ route('profil') }}" wire:navigate
               wire:current="!text-primary-700"
               class="flex items-center gap-2 text-xs text-slate-500 hover:text-slate-900 transition-colors px-2 py-1.5 rounded">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
