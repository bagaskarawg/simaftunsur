...
<div class="min-h-screen flex" x-data="{ sidebarTerbuka: false }">
    ...
    <aside class="w-60 shrink-0 bg-white border-r border-slate-200 flex flex-col fixed inset-y-0 z-30
        ...
        <nav class="flex-1 overflow-y-auto py-3 text-sm">
            ...
            @php $aktif = fn ($name) => request()->routeIs($name); @endphp

            <a href="{{ route('beranda') }}" wire:navigate
               class="flex items-center gap-3 px-4 py-2 font-medium transition-colors border-l-2
                      {{ $aktif('beranda')
                         ? 'bg-primary-50 text-primary-700 border-primary-700'
                         : 'border-transparent text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}"
               @if($aktif('beranda')) aria-current="page" @endif>
            ...
            {{-- Modul-modul ini belum diimplementasikan; tampil sebagai placeholder --}}
            @foreach ([
                ['Data Mahasiswa', 'M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z'],
                ['Klasterisasi',   'M7.5 14.25v2.25m3-4.5v4.5m3-6.75v6.75m3-9v9M6 20.25h12A2.25 2.25 0 0020.25 18V6A2.25 2.25 0 0018 3.75H6A2.25 2.25 0 003.75 6v12A2.25 2.25 0 006 20.25z'],
                ['Prestasi',       'M16.5 18.75h-9m9 0a3 3 0 013 3h-15a3 3 0 013-3m9 0v-3.375c0-.621-.503-1.125-1.125-1.125h-.871M7.5 18.75v-3.375c0-.621.504-1.125 1.125-1.125h.872m5.007 0H9.497m5.007 0a7.454 7.454 0 01-.982-3.172M9.497 14.25a7.454 7.454 0 00.981-3.172M5.25 4.236c-.982.143-1.954.317-2.916.52A6.003 6.003 0 007.73 9.728M5.25 4.236V4.5c0 2.108.966 3.99 2.48 5.228M5.25 4.236V2.721C7.456 2.41 9.71 2.25 12 2.25c2.291 0 4.545.16 6.75.47v1.516M7.73 9.728a6.726 6.726 0 002.748 1.35m8.272-6.842V4.5c0 2.108-.966 3.99-2.48 5.228m2.48-5.492a46.32 46.32 0 012.916.52 6.003 6.003 0 01-5.395 4.972m0 0a6.726 6.726 0 01-2.749 1.35m0 0a6.772 6.772 0 01-3.044 0'],
                ...
                <span class="flex items-center gap-3 px-4 py-2 font-medium text-slate-400 border-l-2 border-transparent cursor-not-allowed"
            ...
            @foreach ([
                ['Pengguna',          'M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z'],
